...
public class AsyncFingerprint extends Handler {
	...
	public static final int VALIDATE_RESULT = 6;
	...
	protected class WorkerHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case PS_LoadChar:
				...
				break;
			case PS_Search:
				Result result = fingerprint.PSSearch(msg.arg1, msg.arg2,
						(Integer) msg.obj);
			...
			case PS_CALIBRATION:
				...
				AsyncFingerprint.this.obtainMessage(PS_CALIBRATION, calibration)
				.sendToTarget();
				break;
			case VALIDATE:
				byte[] data = (byte[]) msg.obj;
				boolean match = validateFinger(data);
				mHandler.obtainMessage(VALIDATE_RESULT, 
						match).sendToTarget();
				break;
			default:
				break;
	...
	private boolean registerFinger() {
		for (int i = 1; i < 3; i++) {
			...
			mHandler.obtainMessage(SHOW_FINGER_IMAGE, image)
					.sendToTarget();
		...
		byte[] model = fingerprint.PSUpChar();
		Log.i("whw", "model length="+model.length);
		mHandler.obtainMessage(SHOW_FINGER_MODEL, model)
				.sendToTarget();
		if (model == null) {
			return false;
		}
