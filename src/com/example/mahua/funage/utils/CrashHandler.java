...
public class CrashHandler implements UncaughtExceptionHandler {
	...
	public void uncaughtException(Thread thread, Throwable ex) {
		...
		// System.exit(10);
		// }
		String errorMsg = genCrashReport(ex);
		Log.e("DefaultExceptionHandler", errorMsg);
		new Thread() {
			@Override
			public void run() {
				Looper.prepare();
	...
	private String genCrashReport(Throwable ex) {
		...
		if (causeEx != null) {
			exceptionStr.append("引发问题的原因如下\n");
			StackTraceElement[] causeElements = ex.getCause().getStackTrace();
			for (int i = 0; i < causeElements.length; i++) {
				exceptionStr.append(causeElements[i].toString() + "\n");
			}
		} else {
			exceptionStr.append("引发问题的原因为空\n");
		}
	...
	private boolean handleException(Throwable ex) {
		...
		// public void run() {
		// new AlertDialog.Builder(mContext).setTitle("提示")
		// .setMessage("程序崩溃了...").setNeutralButton("我知道了",
		// null)
		// .create().show();
		// }
		// });
