...
public class FingerprintAPI extends Handler {
	...
	private static final int BIG_FINGERPRINT_SIZE = 50052;
	private static final int SMALL_FINGERPRINT_SIZE = 40044;
	
//	private static final int CURRENT_FINGERPRINT_SIZE = SMALL_FINGERPRINT_SIZE;
	private static final int CURRENT_FINGERPRINT_SIZE = BIG_FINGERPRINT_SIZE;

	/**
	 * 确认码：0x20—0xef为保留的确认码 表示指令执行完毕或OK；
	 ...
	 *         确认码=03H表示录入不成功 确认码=ffH 表示无响应
	 * Input fingerprint image
	 * Function: probing fingers, detected after the entry stored in the fingerprint image ImageBuffer.
	 * Return Confirmation code said: Entry Success, no fingers.
	 ...
	 *         确认码=ffH表示无响应
	 * Function Description: ImageBuffer the original image to generate the fingerprint profiles stored in CharBuffer1 or CharBuffer2
	 * Input parameters: BufferID (characteristic buffer number)
	 * @param bufferId
	 *            （CharBuffer1:1h,CharBuffer2:2h）
	 * @return Confirmation code 
	 ...
	 *         确认码=ffH 表示无响应
	 * Merge features generate a template that will CharBuffer1 and CharBuffer2 features in the file merge generation templates
	 * The result is stored in CharBuffer1 and CharBuffer2 in.
	 * 
	 * @return Confirmation code 
	 ...
	 * pageId：范围为0~1010 输入参数： BufferID(缓冲区号)，PageID（指纹库位置号）
	 * 
	 * @return 返回值为确认码 确认码=00H表示储存成功 确认码=01H表示收包有错 确认码=0bH表示PageID超出指纹库范围
	 *         确认码=18H表示写FLASH出错 确认码=ffH 表示无响应
	 * The CharBuffer templates stored to the specified number of flash pageId database location bufferId: can only 1h or 2h
	 * pageId：0~1010
	 * Input parameters: BufferID (buffer number), PageID (fingerprint location number)
	 * @return Confirmation code
	 ...
	 *         确认码=0BH表示PageID超出指纹库范围 确认码=ffH 表示无响应
	 ...
	 *         表示无响应
	 * In CharBuffer1 or CharBuffer2 the feature to search the entire or part of the file fingerprint database. If found, it returns the page number.
	 * Input parameters: BufferID, StartPage (Home), PageNum (Pages)
	 * Return parameter: confirm the word, page (match fingerprint templates)
	 * @param bufferId  buffer:1h，2h
	 * @param startPageId 
	 * @param pageNum 
	 * @return
	 * Confirmation code=00H Indicates that the search to;
	 * Confirmation code=01H  An indication that the package is wrong；
	 ...
	 * Exact Match CharBuffer1 the signature file with CharBuffer2 attention point:
	 * the next crew returned data there is also a score when the score is greater
	 * than or equal to 50, the fingerprint matching
	 * 
	 ...
	 * Collect a fingerprint registration template, fingerprint database search space and storage, Storeback pageId
	 * Return parameter: confirm the word, page (match fingerprint templates)
	 * @return
	 * Confirmation code=00H Indicates successful registration;
	 * Confirmation code=01H  An indication that the package is wrong；
	 ...
	 * @return 确认码=00H 表示搜索到； 确认码=01H 表示收包有错； 确认码=09H 表示没搜索到；此时页码与得分为0 确认码=ffH
	 *         表示无响应
	 * Automatic fingerprint in the fingerprint database search target template and return to search results.
	 * If the target template with the current collection of fingerprint matching score greater than the highest threshold,
	 * And the target template is incomplete collection characteristic feature places a blank area to update the target template.
	 *Return parameters: Confirmation code, page number (matching fingerprint template)
	 * @return 
	 * Confirmation code=00H Indicates that the search to;
	 * Confirmation code=01H  An indication that the package is wrong；
	 * Confirmation code=09H Said they were not searched; this time with a score of 0 page
	 ...
	 * Delete Template
	 * Delete a flash ID number specified in the database Start the N fingerprint templates
	 * Input parameters: PageID (fingerprint database template number), N the number of the deleted template.
	 * @param pageIDStart
	 * @param delNum
	 * @return
	 * Confirmation code=00H Success means to delete the template;
	 * Confirmation code=01H  An indication that the package is wrong；
	 ...
	 * @return
	 * Confirmation code=00H Empty successful representation;
	 * Confirmation code=01H  An indication that the package is wrong；
	 ...
	 * The characteristic features of the buffer file uploads to the host computer (the default feature buffer charbuffer1)
	 * 
	...
	public synchronized byte[] PSUpChar() {
		...
		sendCommand(command);
		int length = SerialPortManager.getInstance().read(buffer, 4000, 300);
	 ...
	 *         确认码=ffH 表示无响应
	 *  download the signature file to the module characteristics buffer (the default buffer CharBuffer2)
	 * 
	 * @param model
...
//	 * Upload the image buffer data to the host computer;
//	 * 
...
//		if (length == BIG_FINGERPRINT_SIZE || length == SMALL_FINGERPRINT_SIZE) {
...
//					if(time == 1 || time == 8|| time==15){
//						sendCommand(repeatCommand);
//						Log.i("whw", "time-------------@@@@@@@@@@@@@="+time);
//						time++;
//						size = SerialPortManager.getInstance().readFixedLength(buffer, 3000, requestLength);
//						continue;
//					}
					time++;
					sendCommand(nextCommand);
					...
					isRepeat = true;
					Log.i("whw", "----------image data is lost size=" + size);
					sendCommand(repeatCommand);
					FileUtil.write("----------image data is lost size=" + size);
				}
...
//		System.arraycopy(packet, 0, test, 0, 30);
//		Log.i("whw", "hex str="+DataUtils.toHexString(test));
		...
		do {
			...
			Log.i("whw", "size=" + size + "     packageLength=" + packageLength);
